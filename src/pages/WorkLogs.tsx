  ...
  const loadEmployees = async () => {
    try {
      const { data, error } = await supabase
        .from('employees')
        .select('id, full_name:profiles(full_name), department')
        .order('full_name')
      ...
      const formatted = data?.map(emp => ({
        id: emp.id,
        full_name: emp.full_name?.[0]?.full_name || 'Unknown',
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Alkalmazotti bejelentkezés és kijelentkezés
          </p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Array.from(clockedInEmployees.values()).map(log => {
              ...
              return (
                <div
                  key={log.id}
                  className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-blue-200 dark:border-blue-700"
                >
